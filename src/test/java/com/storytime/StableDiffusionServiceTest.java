...
class StableDiffusionServiceTest {
    ...
    @Test
    void testGenerateImageAndOutputUrl() {
        String apiKey = System.getenv("STABLEDIFFUSION_API_KEY");
        Assumptions.assumeTrue(apiKey != null && !apiKey.isBlank(), "STABLEDIFFUSION_API_KEY is not set");

        StableDiffusionService stableDiffusionService = new StableDiffusionService(new RestTemplate());
        ReflectionTestUtils.setField(stableDiffusionService, "apiKey", apiKey);
